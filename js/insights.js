...
export function buildRuleFindings({ normalized, target, perspective = 'mix', peakDb = null, rmsDb = null, weightedScore = null, roomApplied = false }) {
  ...
  const rms = Number(rmsDb);
  const crest = Number.isFinite(peak) && Number.isFinite(rms) ? peak - rms : null;

  if (Number.isFinite(peak) && peak > -1) findings.push({ severity: 3, title: 'Peak headroom is tight.', text: 'The signal is close to 0 dBFS. Check for clipping before making tonal decisions.' });
  ...
  if (!findings.length && Number.isFinite(Number(weightedScore))) findings.push({ severity: 0, title: 'No rule-level warning.', text: weightedScore >= 80 ? 'The current perspective is translating cleanly.' : 'Use the largest profile mismatch as the next listening target.' });
