...
describe('pre-Play product language contract',()=>{
 it('removes Mix Health from user-facing shell',()=>{ expect(html).not.toMatch(/Mix Health/i); expect(html).not.toMatch(/Target: Balanced mix/i); expect(html).toMatch(/Balance Score/i); });
 it('keeps listening profiles emphasis-only',()=>{
  ...
  expect(html).toMatch(/Focus: /); expect(html).toMatch(/AcelynnBalanceTarget/);
 });
